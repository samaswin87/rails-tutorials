  ...
  <div class="max-w-md m-auto py-10">
    ...
    <form @submit.prevent="addRecord">
      ...
      <div class="mb-6">
        ...
        <p class="pt-4">Don't see an artist? <router-link to="/artists" class="link-gray-500">Create one</router-link></p>
      </div>

      <input type="submit" value="Add Record" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green-500 hover:bg-green-600 block w-full py-4 text-white-500 items-center justify-center">
    </form>
      ...
      <li class="py-4" v-for="record in records" :key="record.id" :record="record">
        <div class="flex items-center justify-between flex-wrap">
          ...
          <p class="block flex font-mono font-semibold flex items-center">
            <svg class="fill-current text-indigo-500 w-6 h-6 mr-2" viewBox="0 0 24 24" width="24" height="24"><title>record vinyl</title><path d="M23.938 10.773a11.915 11.915 0 0 0-2.333-5.944 12.118 12.118 0 0 0-1.12-1.314A11.962 11.962 0 0 0 12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12c0-.414-.021-.823-.062-1.227zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm0-5a1 1 0 1 0 0 2 1 1 0 0 0 0-2z" ></path></svg>
            {{ record.title }} &mdash; {{ record.year }}
          </p>
          <p class="block font-mono font-semibold">{{ getArtist(record) }}</p>
        </div>

        <button class="bg-transparent text-sm hover:bg-blue-500 hover:text-white-500 text-blue-500 border border-blue-500 no-underline font-bold py-2 px-4 mr-2 rounded"
          @click.prevent="editRecord(record)">Edit</button>

        <button class="bg-transparent text-sm hover:bg-red-500 text-red-500 hover:text-white-500 no-underline font-bold py-2 px-4 rounded border border-red-500"
        ...
        <div v-if="record == editedRecord">
          <form action="" @submit.prevent="updateRecord(record)">
            <div class="mb-6 p-4 bg-white rounded border border-gray-400 mt-4">
              ...
              <input type="submit" value="Update" class="bg-transparent text-sm hover:bg-blue-500 hover:text-white-500 text-blue-500 border border-blue-500 no-underline font-bold py-2 px-4 mr-2 rounded">
            </div>
          </form>
        </div>
      </li>
